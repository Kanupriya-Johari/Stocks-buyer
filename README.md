...
| Cloud Server | Azure VM (Standard_B1s) | ~₹600/month |
...
| Time | Action |
|---|---|
| 08:50 AM | Reset daily P&L counters |
| 09:20 AM | Check exits (SL/target hit) |
| 09:25 AM | Scan batch 1 (first 40 symbols) |
| 09:40 AM | Scan batch 2 (next 40 symbols) |
...
### Prerequisites
- Zerodha account with Kite Personal API (free) — [developers.kite.trade](https://developers.kite.trade)
- Twilio account for WhatsApp alerts — [twilio.com](https://www.twilio.com)
- Azure VM (Standard_B1s, Ubuntu 22.04, South India region)

### Installation
```bash
git clone https://github.com/Kanupriya-Johari/Stocks-buyer.git
cd Stocks-buyer
python3 -m venv venv
...
### Running
```bash
# Get daily token first (every market day before 9 AM)
